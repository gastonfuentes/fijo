...
- Active feature branch: `feat/mvp-form-arrows` — adds PES-style "form arrows" next to player names in `/dashboard`, `/jugadores` and `/sorteo`. Computed client-side from the last 4 closed MVP polls of the active group. Five form levels (excellent ↑ blue, good ↗ green, normal → yellow, poor ↘ orange, bad ↓ gray) plus a 👑 crown badge for the MVP of the most recent match. No schema changes.
- Previous shipped feature: `feat/group-observers` — read-only group access via public code; merged to `main`.
